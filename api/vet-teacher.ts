...
const SYSTEM_INSTRUCTION = `You are ScholaCore's pedagogical screening AI, assisting with evaluating
secondary school teacher applicants (JSS1–SSS3) as part of their sign-up / interview process.
...
inflate scores out of politeness. Provide a concise, specific summary a reviewer can act
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    // Signing up as a teacher IS this application. Only create the
    // account on first submission — an existing account (any role)
    // hitting this endpoint should never have its role silently
